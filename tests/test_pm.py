...
def test_the_pm_never_asks_it_briefs_with_what_it_has():
    from renga.design.router import Routing
    assert set(Routing.model_fields) == {"say", "handoffs"}  # nowhere to put a question
    router = Router("main", "pm", DESIGN, model=replying({
        "say": "sent it to design", "handoffs": [{"room": "design", "brief": "a linkedin post for the "
                                                  "new savings account. no date was given: leave it out"}]}))
    lines = run(router, ["person: a linkedin post for the new savings account"])
    assert [(line.kind, line.to) for line in lines] == [("chat", None), ("delegate", "design")]
...
def test_a_handoff_to_a_room_that_isnt_there_is_sent_back():
    router = Router("main", "pm", DESIGN, model=replying(
        {"handoffs": [{"room": "marketing", "brief": "a post"}]},
        {"handoffs": [{"room": "design", "brief": "a post"}]}))
    assert run(router, ["person: a post"])[0].to == "design"


def test_the_pm_is_told_to_brief_from_the_context_and_never_ask():
    seen = []
    ...
    text = seen[0]
    assert "you never ask anyone a question" in text and "never made up" in text
    assert "rates: 4.1%" in text
    assert "never make up a\nfact" in HOUSE_RULES and "no placeholders" in HOUSE_RULES
