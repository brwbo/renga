...
class Job(BaseModel):
    kind: Literal["crew", "router", "listener"]
    room: str
    # a crew
...
def listener_job(room: dict, agents: list[dict], events: list[dict], since: int) -> Job:
    ...
    names = {a["id"]: a["name"] for a in agents}
    lines = _meeting(room, events, names, done=("task", "sent"))
...
def _meeting(room: dict, events: list[dict], names: dict[str, str], done: tuple[str, str],
             you: str | None = None) -> list[tuple[int, str, str]]:
    ...
    reads it; lines of the `done` kind are marked as dealt with."""
    def said(e: dict) -> str:
    ...
    return [(e["id"], e["kind"], said(e)) for e in events
            if e["channel"] == room["id"] and e.get("text")
